...
import com.jeesite.common.web.Servlets;
...
public class UploadUtils4 {
	...
	private static class SingletonHolder {
		private static final UploadUtils4 INSTANCE = new UploadUtils4();
	}

	private UploadUtils4() {
		chunkInfoMap = new ConcurrentHashMap<String, List<String>>();
		// 获取上传临时路径
		tempPath = Servlets.getRequest().getSession().getServletContext().getRealPath(File.separator) + tempPath + File.separator;
		...
		// 文件保存目录路径
		basePath = Servlets.getRequest().getSession().getServletContext().getRealPath(File.separator) + upPath + File.separator;
		// 文件保存目录URL
		baseUrl = Servlets.getRequest().getContextPath() + File.separator + upPath + File.separator;
	...
	public static final UploadUtils4 getInstance() {
	...
	public List<UploadResult> uploadFile(HttpServletRequest request) {
		...
		// 上传
		List<FileItem> fiList = (List<FileItem>) fieldsMap.get(UploadUtils4.FILE_FIELDS);
		Map<String, String> fields = (Map<String, String>) fieldsMap.get(UploadUtils4.FORM_FIELDS);
