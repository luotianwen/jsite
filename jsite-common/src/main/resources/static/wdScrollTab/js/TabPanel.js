...
//    if(iframeObj.contentWindow.frames.length>0)
//    {
...
//      }
//    }
	var subFrame = iframeObj.contentWindow.document.getElementById("subFrame");
	
    if(subFrame != null) {
    	this.iterateFlush(subFrame);
    }
    else
    {
      if(iframeObj.contentWindow.document.forms.length>0)
      {
        for(var i=0; i<iframeObj.contentWindow.document.forms.length; i++)
        {
          try {
            //iframeObj.contentWindow.document.forms[i].submit();
            //iframeObj.contentWindow.document.getElementById("submit_id").click();
        	  
        	 $(iframeObj.contentWindow.document).find("button[type=submit]").trigger("click");
