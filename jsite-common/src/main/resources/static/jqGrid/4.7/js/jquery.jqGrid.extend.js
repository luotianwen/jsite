...
//		multiboxonly: true,	// 单击复选框时在多选
		...
		btnUploader: $('#btnUploader'),					// 文件上传按钮
		uploaderDiv: $('#uploaderDiv'),					// 文件上传布局
...
//					setParam({postData: {parentId: postData.nodeid}});
					var ps = searchForm.serializeArray();
					ps.push({name: "parentId", value: postData.nodeid});
	...
	if (typeof options.btnEventBind == 'function'){
		...
		options.btnSearch.click(function() {
			...
			if (searchForm.hasClass("hide")) {
				...
				// .replace($.jgrid.extend.btnSearch,$.jgrid.extend.btnHideSearch));
			} else {
				searchForm.addClass("hide");
				btnSearch.removeClass("active");
				btnSearch.html(btnSearch.html().replace("隐藏", "查询"));
				// .replace($.jgrid.extend.btnHideSearch,$.jgrid.extend.btnSearch))
			}
		...
		// 绑定文件上传按钮
		options.btnUploader.click(function() {

			var btnUploader = $(this);
		
			if (options.uploaderDiv.hasClass("hide")) {
				options.uploaderDiv.removeClass("hide");
				btnUploader.addClass("active");
				btnUploader.html(btnUploader.html().replace("上传文件", "隐藏"));
				// .replace($.jgrid.extend.btnSearch,$.jgrid.extend.btnHideSearch));
			} else {
				options.uploaderDiv.addClass("hide");
				btnUploader.removeClass("active");
				btnUploader.html(btnUploader.html().replace("隐藏", "上传文件"));
				// .replace($.jgrid.extend.btnHideSearch,$.jgrid.extend.btnSearch))
			}
	...
	function expandTreeNode(level){
		...
		setTimeout(function(){
			...
			if (dataGrid.expandNodeIds.length > 100){
				js.showMessage("本次需要展开“" + dataGrid.expandNodeIds.length
						+ "”个节点，因为展开节点过多，执行取消。");
				dataGrid.expandNodeIds = [];
			}else{
				$('#'+dataGrid.expandNodeIds.shift()+':visible .tree-plus', dataGrid).click();
